...
class PartnerRepo {
  ...
  Future<PartnerContactListModel> getPartnerContactList({String? searchText}) async {
    log("getPartnerContactList list api hit...");
    Response response;
    var token = await SharedPref().getUserToken();
    try {
      dio.options.headers = {
      ...
      response = await dio.get("${ApiPath.partnerContactListAp}?search=$searchText");
      log("getPartnerContactList list response ${response.data.toString()}");
      if (response.statusCode == 200) {
        return PartnerContactListModel.fromJson(response.data);
      } else {
        throw Exception("Faild to load data");
      }
    } catch (e) {
      log("Error while fetch getPartnerContactList list api data ${e.toString()}");
      if (e is DioException) {
        if (e.type == DioExceptionType.connectionTimeout ||
            e.type == DioExceptionType.sendTimeout ||
            e.type == DioExceptionType.receiveTimeout ||
